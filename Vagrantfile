Vagrant.configure("2") do |config|
  config.vm.define "cedar", primary: true do |config|
...
bash <(curl -s https://raw.githubusercontent.com/heroku/stack-images/master/bin/cedar.sh | sed 's/pruned_find -perm .* [|] xargs -r chmod .*//' | sed 's,chown root:root /var/lib/libuuid,,')
